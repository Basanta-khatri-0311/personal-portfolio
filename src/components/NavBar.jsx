...
const NavLink = ({ href, children, isActive }) => {
  return (
    <a
      href={href}
      className={`relative py-2 px-3 font-medium transition-all duration-300 group rounded-lg ${isActive
...
const NavBar = () => {
  ...
  return (
    <>
      ...
      <div className={`md:hidden fixed inset-0 z-40 transition-all duration-400 ease-out ${isOpen
        ? 'opacity-100 pointer-events-auto'
        : 'opacity-0 pointer-events-none'
        }`}>
        {/* Backdrop */}
        <div
          className={`absolute inset-0 bg-black/70 backdrop-blur-sm transition-all duration-400 ${isOpen ? 'opacity-100' : 'opacity-0'
            }`}
          onClick={closeMenu}
        />
        ...
        <div className={`absolute right-0 top-10 h-full w-full bg-gradient-to-b from-gray-900 via-purple-900 to-gray-900 backdrop-blur-3xl transition-transform duration-400 ease-out ${isOpen ? 'translate-x-0' : 'translate-x-full'
          ...
          <div className="flex flex-col items-center justify-center h-full space-y-4 p-6">
            ...
            <div className="w-full space-y-3 mt-16">
              {navItems.map((item, index) => (
                <a
                  key={item}
                  href={`#${item}`}
                  className={`block text-lg font-medium transition-all duration-500 py-3 px-6 rounded-2xl w-full text-center border transform ${isOpen
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className={`w-full text-center px-6 py-3 bg-gradient-to-r from-teal-500 to-blue-600 text-white rounded-full font-medium hover:shadow-xl transform transition-all duration-500 ${isOpen
            ...
            {/* Social Links */}
            <div className={`flex gap-3 pt-6 border-t border-white/10 w-full justify-center transition-all duration-500 ${isOpen ? 'opacity-100' : 'opacity-0'
              }`}
              style={{
              ...
              {[
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`p-2 bg-white/5 rounded-xl border border-white/10 hover:border-teal-400/40 hover:bg-teal-400/10 transition-all duration-300 transform ${isOpen ? 'scale-100 rotate-0' : 'scale-0 rotate-45'
                    }`}
                  style={{
            ...
            {/* Footer text */}
            <div className={`text-center transition-all duration-500 ${isOpen ? 'opacity-100' : 'opacity-0'
              }`}
              style={{
